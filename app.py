...
def calculate_optimal_carry(club_speed, loft, benchmark="Scratch"):
    ...
    Uses interpolated efficiency curve based on LOFT.
    """
...
with st.sidebar:
    ...
    with st.expander(f"📂 Manage Data: {active_user}", expanded=True):
...
if not master_df.empty:
    ...
    filtered_df = filtered_df.copy() # Safe copy
    ...
    with tab_home:
        ...
        render_hero(c_h3, "Total Volume", f"{total_shots}", f"{date_filter}")
        render_hero(c_h4, "Favorite Club", f"{fav_club}", f"{fav_club_count} Shots")
        ...
        if len(filtered_df) > 0:
            ...
            if len(subset) > 0:
                ...
                with c_chart1:
                    ...
                    # 1:1 Aspect Ratio for true dispersion view
                ...
                with c_chart2:
                    st.info("🎯 **Strategy Tip:**\nThe Red Circle represents your 95% miss pattern. If you aim at a flag, ensure there is no trouble within this circle.")
